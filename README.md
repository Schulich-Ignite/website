...
3. `npm install`
4. Make any changes
5. Run ```hugo serve``` and then navigate to [http://localhost:1313/](http://localhost:1313/)
...
Currently NPM is used for:

A) if you're running netlify editor (see above)
B) Running Purge CSS

Purge CSS is an analyzer that will use `hugo_stats.json` (generated by `hugo` at build time) to determine which css is **actually** being used out of our stylesheet, and removing what isn't. This results in a 40-%80 file size reduction in CSS on most pages.

This is invoked using:

```html
{{ $css := resources.Get "css/index.css" | resources.PostCSS | minify | fingerprint | resources.PostProcess }} 
<link rel="stylesheet" href="{{ absURL $css.RelPermalink  }}" integrity="{{ $css.Data.Integrity }}">
```

The CSS variable is assigned using [resource.Get](https://gohugo.io/hugo-pipes/introduction/#:~:text=Get%20locates%20the%20filename%20given%20in%20Hugo%E2%80%99s%20assets%20filesystem%20and%20creates%20a%20Resource%20object%20that%20can%20be%20used%20for%20further%20transformations.%20See%20Get%20Resource%20with%20resources.Get%20and%20resources.GetRemote.) to create a resource object, which has it's content sent through purgeCSS, as well as having an [integriy hash](https://developer.mozilla.org/en-US/docs/Web/Security/Subresource_Integrity) (see [our article](https://schulichignite.com/blog/dangers-of-cdns/#what-is-hashing)) generated, and having it be minified. From there the path is taken and put through absURL in the link tag to convert it to an absolute path (or else this would break at anything with 2 slugs in front i.e. /foo/bar would break the CSS), and the integrity hash is piped into the integrity attribute.
